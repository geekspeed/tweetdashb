...
public class TwitterRiver extends AbstractRiverComponent implements River {
	...
	@Inject
	public TwitterRiver(RiverName riverName, RiverSettings riverSettings,
		...
		if (riverSettings.settings().containsKey("twitter")) {
			Map<String, Object> twitterSettings = (Map<String, Object>) riverSettings
					.settings().get("twitter");

			raw = XContentMapValues.nodeBooleanValue(
					twitterSettings.get("raw"), false);
			// raw = false;
			ignoreRetweet = XContentMapValues.nodeBooleanValue(
					twitterSettings.get("ignore_retweet"), false);
			geoAsArray = XContentMapValues.nodeBooleanValue(
					twitterSettings.get("geo_as_array"), false);

			if (twitterSettings.containsKey("oauth")) {
				Map<String, Object> oauth = (Map<String, Object>) twitterSettings
						.get("oauth");
				if (oauth.containsKey("consumerKey")) {
					...
					oauthConsumerKey = XContentMapValues.nodeStringValue(
							oauth.get("consumerKey"), null);
				} else if (oauth.containsKey("consumer_key")) {
					oauthConsumerKey = XContentMapValues.nodeStringValue(
							oauth.get("consumer_key"), null);
				} else {
					oauthConsumerKey = settings
							.get("river.twitter.oauth.consumer_key");
				}
				if (oauth.containsKey("consumerSecret")) {
					...
					oauthConsumerSecret = XContentMapValues.nodeStringValue(
							oauth.get("consumerSecret"), null);
				} else if (oauth.containsKey("consumer_secret")) {
					oauthConsumerSecret = XContentMapValues.nodeStringValue(
							oauth.get("consumer_secret"), null);
				} else {
					oauthConsumerSecret = settings
							.get("river.twitter.oauth.consumer_secret");
				}
				if (oauth.containsKey("accessToken")) {
					// TODO Remove it in 2.3.0
					logger.warn("accessToken property has been deprecated. Please use access_token instead.");
					oauthAccessToken = XContentMapValues.nodeStringValue(
							oauth.get("accessToken"), null);
				} else if (oauth.containsKey("access_token")) {
					oauthAccessToken = XContentMapValues.nodeStringValue(
							oauth.get("access_token"), null);
				} else {
					oauthAccessToken = settings
							.get("river.twitter.oauth.access_token");
				}
				if (oauth.containsKey("accessTokenSecret")) {
					...
					oauthAccessTokenSecret = XContentMapValues.nodeStringValue(
							oauth.get("accessTokenSecret"), null);
				} else if (oauth.containsKey("access_token_secret")) {
					oauthAccessTokenSecret = XContentMapValues.nodeStringValue(
							oauth.get("access_token_secret"), null);
				} else {
					oauthAccessTokenSecret = settings
							.get("river.twitter.oauth.access_token_secret");
				}
			} else {
				oauthConsumerKey = settings
						.get("river.twitter.oauth.consumer_key");
			...
			if (twitterSettings.containsKey("proxy")) {
				Map<String, Object> proxy = (Map<String, Object>) twitterSettings
						.get("proxy");
				proxyHost = XContentMapValues.nodeStringValue(
						proxy.get("host"), null);
				proxyPort = XContentMapValues.nodeStringValue(
						proxy.get("port"), null);
				proxyUser = XContentMapValues.nodeStringValue(
						proxy.get("user"), null);
				proxyPassword = XContentMapValues.nodeStringValue(
						proxy.get("password"), null);
			} else {
				proxyHost = null;
				proxyPort = null;
				proxyUser = null;
			...
			riverStreamType = XContentMapValues.nodeStringValue(
					twitterSettings.get("type"), "sample");
			Map<String, Object> filterSettings = (Map<String, Object>) twitterSettings
					.get("filter");
			...
			if (filterSettings != null) {
				...
				if (tracks != null) {
					if (tracks instanceof List) {
						...
						filterQuery.track(lTracks.toArray(new String[lTracks
								.size()]));
					} else {
						filterQuery.track(Strings
								.commaDelimitedListToStringArray(tracks
										.toString()));
					}
					filterSet = true;
				}
				...
				if (follow != null) {
					if (follow instanceof List) {
						...
						String[] ids = Strings
								.commaDelimitedListToStringArray(follow
										.toString());
						...
						for (int i = 0; i < lLocations.size(); i++) {
							...
							if (loc instanceof List) {
								...
								if (lLoc.get(0) instanceof Number) {
									...
									lon = Double.parseDouble(lLoc.get(0)
											.toString());
								}
								if (lLoc.get(1) instanceof Number) {
									lat = ((Number) lLoc.get(1)).doubleValue();
								} else {
									lat = Double.parseDouble(lLoc.get(1)
											.toString());
								}
							} else {
								String[] sLoc = Strings
										.commaDelimitedListToStringArray(loc
												.toString());
				...
				if (language != null) {
					if (filterSet) {
						if (language instanceof List) {
							...
							filterQuery.language(lLanguage
									.toArray(new String[lLanguage.size()]));
						} else {
							filterQuery.language(Strings
									.commaDelimitedListToStringArray(language
											.toString()));
						}
					} else {
						streamType = null;
		...
		if (oauthAccessToken == null || oauthConsumerKey == null
		...
		if (riverSettings.settings().containsKey("index")) {
			Map<String, Object> indexSettings = (Map<String, Object>) riverSettings
					.settings().get("index");
	...
	private TwitterStream buildTwitterStream() {
		...
		TwitterStream stream = new TwitterStreamFactory(cb.build())
				.getInstance();
		...
		if (!raw) {
			try {
				...
				if (ExceptionsHelper.unwrapCause(e) instanceof IndexAlreadyExistsException) {
					...
					// event listener here, and only start sampling when the
					// block is removed...
				} else {
					logger.warn(
	...
	String mapping = XContentFactory.jsonBuilder().startObject()
						...
						.startObject("location").field("type", "geo_point").field("index", "not_analyzed")
						.endObject().startObject("language")
						.field("type", "string").field("index", "not_analyzed")
						.endObject().startObject("insights").field("type","string").field("index","not_analyzed")
						.endObject().startObject("user")
						.startObject("properties").startObject("screen_name")
						.field("type", "string").field("index", "not_analyzed")
						.endObject().endObject().endObject()
						.startObject("mention").startObject("properties")
						.startObject("screen_name").field("type", "string")
						.field("index", "not_analyzed").endObject().endObject()
						.endObject().startObject("in_reply")
						.startObject("properties")
						.startObject("user_screen_name")
						.field("type", "string").field("index", "not_analyzed")
						.endObject().endObject().endObject()
			...
			logger.debug("Applying default mapping for [{}]/[{}]: {}",
						indexName, typeName, mapping);
				client.admin().indices().preparePutMapping(indexName)
						.setType(typeName).setSource(mapping).execute()
						.actionGet();
			} catch (Exception e) {
				logger.debug(
	...
	private class StatusHandler extends StatusAdapter {
			...
			try {
				...
				if (status.isRetweet() && ignoreRetweet) {
					if (logger.isTraceEnabled()) {
						logger.trace("ignoring status cause retweet {} : {}",
								status.getUser().getName(), status.getText());
					}
				} else {
					if (logger.isTraceEnabled()) {
						logger.trace("status {} : {}", status.getUser()
					...
					if (raw) {
						...
						bulkProcessor.add(Requests.indexRequest(indexName)
								.type(typeName)
								.id(Long.toString(status.getId())).create(true)
								.source(rawJSON));
					} else {
						XContentBuilder builder = XContentFactory.jsonBuilder()
								.startObject();
						builder.field("text", status.getText());
						String sentiVal = Integer.toString(TweetUtil
								.getSentimentOfTweet(status));
						builder.field("sentival",
								TweetUtil.getSentimentOfTweet(status));
						double[] coord = TweetUtil.getLatLangCache(URLEncoder.encode(status.getUser()
								.getLocation()));
				//		System.out.println(coord+status.getUser().getLocation());
						builder.field("lnglat", coord);


				  List<String> insights = InsightUtil.getInsights(status.getText());
					
						  if(insights.size()!=0){
					
							   for(String s: insights) {
								   System.out.println(s);
								builder.field("insights",s);
									builder.field("issue", s.split(" ")[0]);
							   if(!s.split(" ")[1].equals("GENERIC")){
								   builder.field("flow", s.split(" ")[1]);
							   }else{
								   builder.field("flow", "");
							   }
							   if(!s.split(" ")[2].equals("GENERIC")){
								   builder.field("site_id", s.split(" ")[2]);
							   }else{
								   builder.field("site_id", "");
							   }
						...
						if (status.isRetweet()
							...
							builder.field("id", status.getRetweetedStatus()
									.getId());
							if (status.getRetweetedStatus().getUser() != null) {
								builder.field("user_id", status
										.getRetweetedStatus().getUser().getId());
								builder.field("user_screen_name", status
										.getRetweetedStatus().getUser()
										.getScreenName());
								if (status.getRetweetedStatus()
										.getRetweetCount() != -1) {
									builder.field("retweet_count", status
											.getRetweetedStatus()
											.getRetweetCount());
								}
							}
						...
						if (status.getContributors() != null
								&& status.getContributors().length > 0) {
							builder.array("contributor",
									status.getContributors());
						}
						if (status.getGeoLocation() != null) {
							if (geoAsArray) {
								builder.startArray("location");
								builder.value(status.getGeoLocation()
										.getLongitude());
								builder.value(status.getGeoLocation()
										.getLatitude());
								builder.endArray();
								...
								builder.field("lat", status.getGeoLocation()
										.getLatitude());
								builder.field("lon", status.getGeoLocation()
										.getLongitude());
								builder.endObject();
							}
						}
						if (status.getPlace() != null) {
							...
							builder.field("type", status.getPlace()
									.getPlaceType());
							builder.field("full_name", status.getPlace()
									.getFullName());
							builder.field("street_address", status.getPlace()
									.getStreetAddress());
							builder.field("country", status.getPlace()
									.getCountry());
							builder.field("country_code", status.getPlace()
									.getCountryCode());
							...
							for (URLEntity url : status.getURLEntities()) {
								if (url != null) {
									...
									if (url.getDisplayURL() != null) {
										builder.field("display_url",
												url.getDisplayURL());
									}
									if (url.getExpandedURL() != null) {
										builder.field("expand_url",
												url.getExpandedURL());
									}
						...
						builder.field("screen_name", status.getUser()
								.getScreenName());
						builder.field("location", status.getUser()
								.getLocation());
						builder.field("description", status.getUser()
								.getDescription());
						builder.field("profile_image_url", status.getUser()
								.getProfileImageURL());
						builder.field("profile_image_url_https", status
		...
		public void onTrackLimitationNotice(int numberOfLimitedStatuses) {
			logger.info(
					"received track limitation notice, number_of_limited_statuses {}",
